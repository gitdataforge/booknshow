...
export default function AdminAddMatch() {
    ...
    const handleTestEmail = async () => {
        ...
        const res = await sendParbetEmail({ to_name: 'Admin', message: `Ping from AdminAddMatch.` });
    ...
    return (
        <div className="w-full max-w-5xl mx-auto p-4 md:p-8 animate-fade-in">
            ...
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex justify-between items-center mb-8">
                ...
                <h1 className="font-bold text-xl tracking-wide">AdminAddMatch</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-24 md:pb-8">
                ...
                <motion.div whileHover={{ scale: 0.99 }} className="bg-brand-card p-6 rounded-[24px] border border-white/5 shadow-lg md:col-span-2 bg-gradient-to-r from-brand-card to-brand-primary/10">
                    ...
                    <p className="text-sm text-brand-muted mb-6 max-w-md">Access premium features, manage your secure wallet, and control your global Parbet settings within the AdminAddMatch module.</p>
                ...
                <div className="bg-brand-card rounded-[24px] border border-white/5 p-6">
                    ...
                    <p className="text-xs text-brand-muted mb-6 leading-relaxed">Toggle functionalities for the AdminAddMatch environment. Changes sync instantly via Firestore rules.</p>
