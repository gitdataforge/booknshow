...
import { useMainStore } from '../store/useMainStore'; // INJECTED: Global Store
 ...
 * FEATURE 5: Double-Footer Bug Resolution
 * FEATURE 6: Fullscreen State Subscription (Hides sidebar when modals are open)
 */
...
export default function ProfileLayout() {
    ...
    // Global State Subscription for Fullscreen Modals
    const { isFullscreenModalOpen } = useMainStore();
    ...
    // FEATURE 6: Force collapse/hide sidebar if a fullscreen modal is actively blocking the UI
    const isSidebarHidden = isFullscreenModalOpen;

    return (
        <div className="flex flex-col w-full min-h-screen bg-[#FFFFFF] font-sans relative">
            
            {/* INJECTED PROFILE HEADER (Also hidden if fullscreen modal is open) */}
            {!isFullscreenModalOpen && <ProfileHeader />}

            {/* Container for Sidebar and Main Content */}
            <div className={`flex w-full flex-1 ${isFullscreenModalOpen ? 'mt-0' : 'mt-[72px]'}`}>
                
                {/* SECTION 1: Desktop Collapsible Sidebar (Conditionally Hidden) */}
                <AnimatePresence>
                    {!isSidebarHidden && (
                        <motion.aside 
                ...
                {/* Expanding wrapper limits if fullscreen modal is open */}
                <main className={`flex-1 w-full bg-[#FFFFFF] relative flex flex-col min-h-[calc(100vh-72px)] overflow-x-hidden z-10 ${isFullscreenModalOpen ? 'max-w-full p-0 m-0' : 'max-w-[1200px] p-5 md:p-10 mx-auto'}`}>
            ...
            {/* GLOBAL INJECTION: Persistent Real-Time Feedback Tab (Hidden during fullscreen) */}
            {!isFullscreenModalOpen && <FeedbackTab />}
