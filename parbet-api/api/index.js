        ...
        if (routeName === 'resetPassword' || routeName === 'sendVerification') {
            ...
            try {
                if (routeName === 'resetPassword') {
                    ...
                    if (error) return res.status(500).json({ error: 'Resend API failed to dispatch.' });
                    ...
                    if (error) return res.status(500).json({ error: 'Resend API failed to dispatch.' });
                    return res.status(200).json({ success: true, message: 'Verification link dispatched.' });
                }
            } catch (error) {
                console.error('Critical Core Auth Pipeline Failure:', error);
                return res.status(500).json({ error: 'Failed to process authentication request.' });
            }
            return; // Terminate execution to prevent falling into dynamic router
        }
